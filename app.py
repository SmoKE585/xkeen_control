...
from config import APP_ICON_ICO, STATUS_POLL_INTERVAL, TRAY_ICON_OFF, TRAY_ICON_ON, TRAY_ICON_UNKNOWN, format_host
from services.editor_sync import ConfigEditorSync
...
class VPNTrayApp:
    ...
    def __init__(self):
        ...
        self.editor_sync = ConfigEditorSync(self.router.ssh)
        ...
        self.signals.tray_tooltip_changed.connect(self.tray.setToolTip)
        ...
        self.config_sync_timer.timeout.connect(lambda: self.editor_sync.sync(self.signals.error_occurred.emit))
    ...
    def _setup_tray_menu(self) -> None:
        menu = QMenu()
        for text, handler in (
            ("Открыть", self.action_show),
            ("Включить", self.action_on),
            ("Выключить", self.action_off),
            ("Перезапустить", self.action_restart),
            ("Настройки", self.action_settings),
            ("Проверить статус", self.action_check),
            ("Выход", self.action_exit),
        ):
            if text in {"Включить", "Настройки", "Выход"}:
                menu.addSeparator()
    ...
    def _compose_tray_title(self, status: str) -> str:
        ...
        try:
            checks = self.router.get_connectivity_checks()
    ...
    def _apply_status(self, new_status: str) -> None:
        with self.lock:
            ...
            self.tray.setIcon({"on": self.icon_on, "off": self.icon_off}.get(new_status, self.icon_unknown))
    ...
    def get_notepadpp_path(self):
        return self.router.get_notepadpp_path()

    def run_config_editor(self, path: str) -> None:
        self.editor_sync.open_for_edit(path, self.get_notepadpp_path())
        if not self.config_sync_timer.isActive():
            self.config_sync_timer.start()

    def load_outbounds_config(self) -> dict:
        return self.router.load_outbounds_config()

    def save_outbounds_config(self, data: dict) -> None:
        self.router.save_outbounds_config(data)

    def load_routing_config(self) -> dict:
        return self.router.load_routing_config()

    def save_routing_config(self, data: dict) -> None:
        self.router.save_routing_config(data)

    def load_site_checks(self) -> list[dict]:
        return self.router.load_site_checks()

    def save_site_checks(self, checks: list[dict]) -> None:
        self.router.save_site_checks(checks)

    def _startup_command(self) -> str:
        if getattr(sys, "frozen", False):
            return f'"{sys.executable}"'
        return f'"{sys.executable}" "{os.path.join(os.path.dirname(os.path.abspath(__file__)), "vpn_tray.py")}"'
    ...
    def reload_configs(self) -> None:
        try:
            self.signals.configs_changed.emit(self.router.list_configs())
        except Exception as exc:
            self.signals.configs_changed.emit([f"[ошибка] {exc}"])
    ...
    def action_on(self) -> None:
        self.run_vpn_action_async(self.router.vpn_on, "Запуск xkeen")

    def action_off(self) -> None:
        self.run_vpn_action_async(self.router.vpn_off, "Остановка xkeen")

    def action_restart(self) -> None:
        self.run_vpn_action_async(self.router.vpn_restart, "Перезапуск xkeen")
    ...
    def poll_status_loop(self) -> None:
        while not self.stop_event.is_set():
            try:
                self.update_icon_status(self.router.get_status())
            except Exception:
    ...
    def run(self) -> None:
        initial_status = self.router.get_status()
